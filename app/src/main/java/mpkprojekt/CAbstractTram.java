...
public abstract class CAbstractTram implements Objects {
    ...
    CPosition headPosition() {                                                  // zwraca pozycje toru na podstawie pozycji na liscie torow przechowywanych w linii
    ...
    boolean checkIfCanGo(){                             // sprawdza czy tramwaj moze sie ruszyc
        return true;                                        // rozszerzana w klasach potomnych
    }

    public void move(){                                                         // przemieszcza tramwaj o jedna pozycje
    ...
    public ArrayList<CPosition> listToDraw() {                                  // zwraca liste pozycji na mapie, na ktorych znajduje sie tramwaj
    ...
    public ArrayList<CPosition> listToCheckColision() {
        ArrayList<CPosition> positions = new ArrayList<>();                     // zwraca liste pozycji, zajetych przez aktualny trmawaj
        for (int i = 0; i <= length; i++) {                                     // dziala podobnie do listToDraw
            int x = (map_pos - i+line.tracks.size())%line.tracks.size();        // dodatkowo zwraca jeszcze jedna pozycje w tyl,
            positions.add(line.tracks.get(x).pos);                              // aby trawaje utrzymywaly jedna pozycji przerwy
        }
        return positions;
    }
}